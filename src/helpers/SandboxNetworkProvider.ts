...
class SandboxTonClient extends TonClient {
  ...
  override async runMethod(
    ...
    return {
      ...
      stack: new TupleReader(result.stack as TupleItem[]),
    };
  }

  override async callGetMethod(
    address: Address,
    name: string,
    stack: TupleItem[] = [],
  ): Promise<{ gas_used: number; stack: TupleReader }> {
  ...
  override async runMethodWithError(
    address: Address,
    name: string,
    params: any[] = [],
  ...
  override async callGetMethodWithError(
    address: Address,
    name: string,
    stack: TupleItem[] = [],
  ): Promise<{ gas_used: number; stack: TupleReader }> {
  ...
  override async tryLocateResultTx(
    source: Address,
    destination: Address,
    created_lt: string,
  ): Promise<Transaction> {
  ...
  override async tryLocateSourceTx(
    source: Address,
    destination: Address,
    created_lt: string,
  ): Promise<Transaction> {
  ...
  override async getShardTransactions(
    workchain: number,
    seqno: number,
    shard: string,
  ): Promise<any> {
  ...
  override async estimateExternalMessageFee(
    address: Address,
    args: {
      body: Cell;
      initCode: Cell | null;
  ...
  override async sendExternalMessage(
    contract: Contract,
    src: Cell,
  ): Promise<void> {
  ...
  override async getContractState(address: Address): Promise<{
    balance: bigint;
    extra_currencies:
    ...
    return {
      ...
      state: resolvedState,
      code:
        acc.accountState?.type === "active"
          ? (acc.accountState.state.code?.toBoc() ?? null)
          : null,
      data:
        acc.accountState?.type === "active"
          ? (acc.accountState.state.data?.toBoc() ?? null)
          : null,
  ...
  override open<T extends Contract>(src: T): OpenedContract<T> {
    return this.blockchain.openContract(
      src as unknown as SandboxContract<T>,
...
export class SandboxNetworkProvider implements NetworkProvider {
  ...
  constructor(
    ...
    this.internalUi = {
      ...
      inputAddress: async (message: string): Promise<Address> => {
        ...
        console.log("mock send address " + address);
      ...
      setActionPrompt: (message: string): void => {
        console.log("[UI Set Action]", message);
      },
      clearActionPrompt: (): void => {
        console.log("[UI Clear Action]");
      },
    };
  ...
  provider(
    ...
    console.warn(
      "SandboxNetworkProvider.provider() returning Blockchain instance, not a true ContractProvider wrapper.",
    );
    return this.blockchain as unknown as ContractProvider;
  }
  ...
  open<T extends Contract>(contract: T): OpenedContract<T> {
    const opened = this.blockchain.openContract(
      contract as unknown as SandboxContract<T>,
    );
  ...
  async waitForDeploy(
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      if (contractState.state === "active") {
        this.internalUi.write(
          `Contract ${addr.toString()} deployed and active.`,
        );
        return;
      }
    ...
    throw new Error(
      `Contract ${addr.toString()} did not deploy within ${attempts} attempts.`,
    );
  }
  ...
  async deploy(
    ...
    console.log(
      `Mock Deploying ${contract.address.toString()} via internal message with value ${value.toString()}...`,
    );

    const _init: StateInit | undefined = contract.init
      ? { code: contract.init.code, data: contract.init.data }
      : undefined;
    ...
    console.log(
      `Mock Deploy internal message sent for ${contract.address.toString()}.`,
    );
