...
export const generateSpecsRunner = async (args: Args, ui: UIProvider) => {
  ...
  const defaultProviderPath = path
    .relative(testsDir, "src/helpers/SandboxNetworkProvider")
    .replace(/\\/g, "/")
    .replace(/\.ts$/, "");
  const providerPathInput =
    (options["provider-path"] as string) ?? defaultProviderPath;
  // Remove .ts extension if present, as TS/JS imports don't require it
  const providerPath = providerPathInput.endsWith(".ts")
    ? providerPathInput.slice(0, -3)
    : providerPathInput;
